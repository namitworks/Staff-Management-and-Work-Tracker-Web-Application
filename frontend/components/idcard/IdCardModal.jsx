  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md sm:max-w-2xl bg-slate-50">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold text-slate-800">Staff ID Card - {staff.name}</DialogTitle>
          <p className="text-sm text-slate-500 mt-1">
            Staff ID: <span className="font-semibold text-slate-700">{staff.staff_id}</span> | Valid Until:{' '}
            <span className="font-semibold text-slate-700">{formatValidUntil(staff.valid_until)}</span>
          </p>
        </DialogHeader>
        ...
        <div className="flex flex-col items-center mt-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 w-full">
            <Button variant="outline" onClick={() => handleDownloadImage('front')} disabled={isDownloadingFront} className="text-xs">
              {isDownloadingFront ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Download className="w-4 h-4 mr-2" />}
              Front (PNG)
            </Button>

            <Button variant="outline" onClick={() => handleDownloadImage('back')} disabled={isDownloadingBack} className="text-xs">
              {isDownloadingBack ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Download className="w-4 h-4 mr-2" />}
              Back (PNG)
            </Button>

            <Button onClick={handleDownloadPdf} disabled={isDownloadingPdf} className="text-xs bg-[#1A3A5C] hover:bg-[#2E75B6]">
